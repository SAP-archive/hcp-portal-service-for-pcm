(function() {
	...
	sap.ui.controller("c4c.details-leadcollection.local.view.details", {
		...
		valueHelpersMetaData: {
			...
			"LeadItem": {
				"ProductID": {
					...
					"comboBox": false,
					"BusinessObject": "LeadItemCollection",
					"prefixKey": "CHP.LeadItem.ProductID.Common.ValueList",
					"titleKey": "CHP.LeadItem.UI.LineItem.UI.DataField.ProductID",
					"fieldName": "ProductID"
				},
				"combobox": {
					"QuantityUnitOfMeasure": {
						"CollectionPath": "LeadItemQuantityUnitOfMeasure",
						"searchSupported": false,
						"viewParams": {
							"Code": [
								"QuantityUnitOfMeasure"
			...
			"LeadAttachment": {
				"combobox": {
					"CategoryCode": {
						"CollectionPath": "LeadAttachmentCategoryCode",
						"searchSupported": false,
						"viewParams": {
							"Code": [
								"CategoryCode"
						...
						"filterable": [],
						"comboBox": true,
						"BusinessObject": "LeadAttachmentCollection",
						"enabled": true
					}
				}
			}
		...
		updateFields: ["ObjectID", "Category", "City", "Company", "ContactFirstName", "ContactLastName", "Country", "EndDate", "HouseNumber",
			"Name", "PostalCode", "Priority", "StartDate", "State", "Status", "Street", "UserStatusCode"
		],
		openValueHelpDialog: function openValueHelpDialog(oController) {
			...
			var oValueHelpDialog = sap.ui.controller("c4c.details-leadcollection.local.view.vhelpdialog");
		...
		loadData: function(collectionId) {
			...
			oDataServiceModel.attachMetadataLoaded({}, function() {
				...
				this.readSubObject(oDataServiceModel, collectionApi, "LeadItem", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "LeadContact", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "LeadInvolvedParty", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "LeadSalesAndMarketingTeam", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "LeadConversion", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "LeadAttachment", oJsonModel, counter);
		...
		openActions: function openActions(oEvent) {
			...
			if (!this.actionSheet) {
				this.actionSheet = sap.ui.xmlfragment(
					"c4c.details-leadcollection.local.view.actions.actionsSheet",
					this
				);
		...
		handleOpenDialog: function(oEvent) {
			this.oDialog = sap.ui.xmlfragment("c4c.details-leadcollection.local.view.dialog.delete", this);
		...
		showAddDialog: function showAddDialog() {
			...
			var fragmentName = "c4c.details-leadcollection.local.view.dialog." + this.tableName + "Add";
		...
		onEditFacetItem: function onEditFacetItem(oEvent) {
			...
			var fragmentName = "c4c.details-leadcollection.local.view.dialog." + this.tableName + "Update";
			this.oDialog = sap.ui.xmlfragment(fragmentName, this);
			this.getView().addDependent(this.oDialog);
		...
		getFormFragment: function(sFragmentName) {
			...
			oFormFragment = sap.ui.xmlfragment(
				this.getView().getId(),
				"c4c.details-leadcollection.local.view.details" + sFragmentName,
				this
			);
