sap.ui.define(["sap/ui/core/UIComponent", "sap/ui/model/odata/ODataUtils"], function(UIComponent) {
	return UIComponent.extend("c4c.table-leadcollection.local.Component", {
		...
		configuration: {
			componentName: "/Lead/LeadTable",
			applicationPath: "/pcmapps",
			fioriPrefix: "/sap/fiori",
			appServiceSuffix: "/sap/c4c/odata/v1/pcmportal",
			businessObject: {
				targetEntity: "LeadCollection",
				semanticObject: "Lead",
				growingThreshold: 15,
				queries: [{
					name: "CHP.Lead.UI.Query.UI.Filter.AllLeads",
					type: "Parameter",
					isDefault: "true",
					filter: []
				}, {
					name: "CHP.Lead.UI.Query.UI.Filter.ConvertedLeads",
					type: "Parameter",
					isDefault: "false",
					filter: [{
						key: "UserStatusCode",
					...
					name: "CHP.Lead.UI.Query.UI.Filter.MyLeads",
					funcName: "Get_My_Leads",
					type: "FunctionImport",
					isDefault: "false",
					filter: []
				}]
		...
		setModulePath: function() {
			...
			try {
				if (sap.ushell.services.AppConfiguration) {
					...
					jQuery.sap.registerModulePath("c4c.table-leadcollection.local", this.configuration.appName);
					jQuery.sap.registerModulePath("c4c.table-leadcollection.remote", this.configuration.appName);
				} else {
		...
		createContent: function() {
			...
			var appEventsBus = new sap.ui.core.EventBus(),
				dialogView = sap.ui.view({
					viewName: "c4c.table-leadcollection.local.view.dialog",
				...
				view = sap.ui.view({
					viewName: "c4c.table-leadcollection.local.view.table",
					type: sap.ui.core.mvc.ViewType.XML,
					viewData: {
