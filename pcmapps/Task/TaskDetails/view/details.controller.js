(function() {
	...
	sap.ui.controller("c4c.details-taskcollection.local.view.details", {
		...
		valueHelpersMetaData: {
			...
			"TaskAttachment": {
				"combobox": {
					"CategoryCode": {
						"CollectionPath": "TaskAttachmentCategoryCode",
						"searchSupported": false,
						"viewParams": {
							"Code": [
								"CategoryCode"
						...
						"filterable": [],
						"comboBox": true,
						"BusinessObject": "TaskAttachmentCollection",
						"enabled": true
					}
				}
			}
		...
		updateFields: ["ObjectID", "ActivityTypeCode", "Category", "Owner", "Priority", "Status", "Subject", "TaskDueDateTime",
			"TaskStartDateTime"
		],
		openValueHelpDialog: function openValueHelpDialog(oController) {
			...
			var oValueHelpDialog = sap.ui.controller("c4c.details-taskcollection.local.view.vhelpdialog");
		...
		loadData: function(collectionId) {
			...
			oDataServiceModel.attachMetadataLoaded({}, function() {
				...
				this.readSubObject(oDataServiceModel, collectionApi, "TaskInvolvedParty", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "TaskNote", oJsonModel, counter);
				this.readSubObject(oDataServiceModel, collectionApi, "TaskAttachment", oJsonModel, counter);
		...
		openActions: function openActions(oEvent) {
			...
			if (!this.actionSheet) {
				this.actionSheet = sap.ui.xmlfragment(
					"c4c.details-taskcollection.local.view.actions.actionsSheet",
					this
				);
		...
		handleOpenDialog: function(oEvent) {
			this.oDialog = sap.ui.xmlfragment("c4c.details-taskcollection.local.view.dialog.delete", this);
		...
		showAddDialog: function showAddDialog() {
			...
			var fragmentName = "c4c.details-taskcollection.local.view.dialog." + this.tableName + "Add";
		...
		onEditFacetItem: function onEditFacetItem(oEvent) {
			...
			var fragmentName = "c4c.details-taskcollection.local.view.dialog." + this.tableName + "Update";
			this.oDialog = sap.ui.xmlfragment(fragmentName, this);
			this.getView().addDependent(this.oDialog);
		...
		getFormFragment: function(sFragmentName) {
			...
			oFormFragment = sap.ui.xmlfragment(
				this.getView().getId(),
				"c4c.details-taskcollection.local.view.details" + sFragmentName,
				this
			);
