sap.ui.define(["sap/ui/core/UIComponent", "sap/ui/model/odata/ODataUtils"], function(UIComponent) {
	return UIComponent.extend("c4c.table-taskcollection.local.Component", {
		...
		configuration: {
			...
			businessObject: {
				...
				growingThreshold: 15,
				queries: [{
					name: "CHP.Task.UI.Query.UI.Filter.AllTasks",
					type: "Parameter",
					isDefault: "true",
					filter: []
				}, {
					name: "CHP.Task.UI.Query.UI.Filter.MyOpenTasks",
					funcName: "Get_My_Open_Tasks",
					type: "FunctionImport",
					isDefault: "false",
					filter: []
				}]
		...
		setModulePath: function() {
			...
			try {
				if (sap.ushell.services.AppConfiguration) {
					...
					jQuery.sap.registerModulePath("c4c.table-taskcollection.local", this.configuration.appName);
					jQuery.sap.registerModulePath("c4c.table-taskcollection.remote", this.configuration.appName);
				} else {
		...
		createContent: function() {
			...
			var appEventsBus = new sap.ui.core.EventBus(),
				dialogView = sap.ui.view({
					viewName: "c4c.table-taskcollection.local.view.dialog",
				...
				view = sap.ui.view({
					viewName: "c4c.table-taskcollection.local.view.table",
					type: sap.ui.core.mvc.ViewType.XML,
					viewData: {
